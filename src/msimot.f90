...
!---------------------------------------------------------------------
subroutine varotn(v, nvar, ivar, nameptrs, names, nodec, sortmvn, mathdr)


!     Output a number of model variables with a header mathdr.

!     v(*)       :: the values of the variables
!     nvar       :: number of variables to print.
!     ivar       :: index of the variables to print
!                   (v(ivar(i)), i=1, nvar  is printed).
!     nameptrs   :: name pointer array. nameptr(ivar(i))
!                   points to the name of the i'th printed variable
!     names      :: name string memory.
!     nodec      :: number of decimals to print.
!     sortmvn    :: if true, then model variable names are sorted
!                   alphabetically.
!     mathdr     :: string which contains a header.
...
implicit none
real*8, intent(in) :: v(*)
integer(kind = MC_IKIND), intent(in) ::  nvar, ivar(*)
integer, intent(in) ::  nodec
logical, intent(in)      ::  sortmvn
integer(kind = MC_IKIND), intent(in) ::  nameptrs(*), names(*)
character*(*), intent(in):: mathdr

!     colwid  column width
!     ncols  number of columns
!     intWid maximum number of digits before decimal point.
!     numWidV field width for printing numbers
!     noEfmt number of digits printed when ES format is used.

integer   :: nameLen, intWid, numWidV, noEfmt
integer   :: colwid, ncol , i, j, jb, je
real*8  :: vmax, x
real*8, allocatable :: temp(:)

! sort model variable names
if (sortmvn) call hsortmvn(ivar, nvar, nameptrs, names)

noEfmt = max(MinNoEfmt, nodec + 1)

! get length longest name
namelen = maxnli(nameptrs, ivar, 1_MC_IKIND, nvar)

! get the largest value printed using F format
vmax = Rzero
do i = 1, nvar
    x = abs(v(ivar(i)))
    if (useFfmt(x, MaxIntWid, nodec) .and. x .gt. vmax) vmax = x
enddo

! determine the number of digits before the decimal point for the
! largest number printed with F format.
intWid = 1
do i = 1 , MaxIntWid - 1
    vmax = int(vmax) / 10
    if (vmax .eq. Rzero) exit
    intWid = intWid + 1
enddo

! Increase numwid if required
numWidV = intWid + nodec + 1
do i = 1, nvar
    numWidV = max(numWidV, minNumWid(v(ivar(i)), intWid, nodec))
enddo

colwid = max(namelen, numWidV)
ncol   = max(1, LWIDTH / (colwid + ICSPAC) )
...
do jb = 1, nvar, ncol
   ...
   je   = min(nvar, jb + ncol - 1)
   do j = jb , je
      temp(j - jb + 1) = v(ivar(j))
   enddo

!        insert variable names right justified within column width
   call strini( ' ', 1)
   call snmfmt(names, nameptrs, ivar, jb, je, colwid)
   call strout(O_OUTB)

!        print data values
   call strini( ' ', 1)
   call svlvfmt(temp, 1, je - jb + 1, colwid, numWidV, MaxIntWid, nodec, noEfmt)
   call strout(O_OUTB)
   call strini( ' ', 1 )
   call strout(O_OUTB)
...
return
end subroutine varotn

!-----------------------------------------------------------------------

subroutine tsotn(ts, ldts, jb, je,  nvar,  ivar, nameptrs, &
&                 names, nodec, columns, sortmvn, mathdr, error)

!     Output a number of timeseries rowwise with a header mathdr.

!     ts         :: a two-dimensional matrix with the timeseries
!                   the columns contain the timeseries.
!                   the first colunm contains the timeseries
!                   for time jb
!     ldts       :: leading dimension of ts.
!     jb         :: first period
!     je         :: last period.
!     nvar       :: number of variables to print.
!     ivar       :: index of the variables to print
!                   (ts(:, ivar(i)), i=1, nvar  is printed).
!     nameptrs   :: name pointer array. nameptr(ivar(i))
!                  points to the name of the i'th printed variable
!     names          :: name string memory.
!     nodec      :: number of decimals to print.
!     columns    :: max. number of columns to print
!                   (if -1, then determined by line width).
!     sortmvn    :: if true, then model variable names are sorted
!                   alphabetically.
!     mathdr     :: string which contains a header.

!     Output : error (logical). Set to .true. if not enough
...
use nuna
use nucnst
integer, intent(in)       :: ldts, jb, je, nodec, columns
integer(kind = MC_IKIND), intent(in) :: nvar, ivar(*), nameptrs(*), names(*)
logical, intent(in)       :: sortmvn
character*(*), intent(in) :: mathdr
real*8, intent(in)        :: ts(ldts,*)
real*8                    :: x, xmax
logical, intent(out)      :: error

integer            ::  nj, colwid, maxrhl, maxchl
integer            ::  ncpb,ncpe,npcols
integer            ::  numWidV, intWid, noEfmt
integer            ::  i, j, j1, j2, stat
real*8, allocatable  :: temp(:)
...
!     nj        number of periods
!     maxrhl    length of longest row header
!     maxchl    length of longest column header
!     colwid    column width
!     npcols    number of columns in each strip
!     ncpb      index of first column to print
!     ncpe      index of last  column to print
!     intWid    maximum number of digits before decimal point.
!     numWidV   field width for printing numbers
...
nj      = max(je - jb + 1, 0)
maxrhl  = maxnli(nameptrs, ivar, 1_MC_IKIND, nvar)
maxchl  = maxperlen()
noEfmt = max(MinNoEfmt, nodec + 1)

! get the largest data value printed using F format
xmax = Rzero
do i = 1, nvar
    do j = jb, je
        x = abs(ts(j - jb + 1, ivar(i)))
        if (      .not. nuifna(x) .and. useffmt(x, MaxIntWid, nodec) &
&           .and. x .gt. xmax                      ) then
           xmax = x
        endif
    enddo
...
! determine the number of digits before the decimal point for the
! largest number printed using F format
intWid = 1
do i = 1 , MaxIntWid - 1
    xmax = int(xmax) / 10
    if (xmax .eq. Rzero) exit
    intWid = intWid + 1
enddo
...
do i = 1, nvar
    do j = jb, je
        x  = ts(j - jb + 1, ivar(i))
        numWidV = max(numWidV, minNumWid(x, intWid, nodec))
    enddo
enddo

colwid = max(maxchl, numWidV)

if (columns .gt. 0) then
    npcols  = min(columns, (LWIDTH - maxrhl - 1) / (colwid + ICSPAC) )
else
    npcols  = (LWIDTH - maxrhl - 1) /(colwid + ICSPAC)
endif

allocate(temp(npcols), stat = stat)
if (stat /= 0) then
    error = .true.
    return
endif
...
do ncpb = 1, nj, npcols
     ...
     ncpe = min(ncpb + npcols - 1, nj)
     j1   = ncpb - 1 + jb
     j2   = ncpe - 1 + jb

!          write col headers (right justified)
     call strini(' ', 1 + maxrhl + ICSPAC)
     call sperfmt(j1, j2, colwid)
     call strout(O_OUTB)
     ...
     do i = 1, nvar
        ...
        do j = ncpb, ncpe
           temp(j-ncpb+1) = ts(j, ivar(i))
        enddo

!             write row header (left justified)
        call mcf7ex(name, nlen, nameptrs(ivar(i)), names)
        call strini(name(:nlen),1 + maxrhl + ICSPAC)

!             write x(i,j=ncpb..ncpe) (right justified)
        call svlvfmt(temp, 1, ncpe-ncpb+1, colwid, &
&         numWidV, MaxIntWid, nodec, noEfmt)
        call strout(O_OUTB)
