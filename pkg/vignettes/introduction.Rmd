...
used to solve a simple example model, 
...
* `islm.mif` The model information file (mif file). This  is a binary file. It
contains the internal code for the evaluation of the model equations.
...
The mrf file contains technical information about the model. For example,
it shows the feedback variables of the model. It is important to know
...
To supply the required data to the model, we first define `regts` 
timeseries objects. For the exogenous variables `g` and `ms` we
...
`names` to supply the timeseries names.
