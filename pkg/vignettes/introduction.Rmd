...
Function `copy_example_mdl` is a convenience
function that can be used to copy these example models to your working directory.
For the ISLM model, use
```{r, results = "hide"}
copy_example_mdl("islm")
```
The command above copies the islm model to file `islm.mdl` in the current 
working directory.
...
mdl$get_param("m0")
```
Without the argument `deep = TRUE`, the result of the `clone` method
is the same as the result of an assignment `mdl2 <- mdl`.
...
method `set_period()`:
```{r}
mdl$set_period(regperiod_range("2017Q1", "2018Q2"))
mdl
```
