...
The end statement is not allowed in files included with the `#include` 
preprocessor directive (see Section \ref{sec:include}). 
Instead of the end statement, it is possible to use conditional
compilation using a dummy flag (see \ref{sec:skip_equations}).
...
It is not allowed to use the end statement (see Section \ref{sec:end}) in an included
file. If you wish to skip some equations at the end of the file,
use conditional compilation (see section \ref{sec:skip_equations}) with a dummy
flag.
...
### Using conditional compilation to skip equations
\label{sec:skip_equations}

Conditional compilation can also be used to skip some equations during
...
If the compiler flag `dummy` has not been defined, the equations `x = 1` and
`z = 2` are skipped.
