...
test_that("fixed fit instruments (6)", {
  mdl <- islm_model$copy()
  y <- regts(200, period = '2015q2/2016q3')
  r <- regts(3.5, period = '2015q2/2016q3')
  fit_targets <- cbind(y, r)
  mdl$set_fit(fit_targets)
  mdl$fix_variables("t", period = "2015q2/2016q3")
  mdl$fix_variables("c", period = "2015q3")
  mdl$fix_variables("i", period = "2016q1")
  mdl$set_fit_options(zealous = FALSE, warn_ca = FALSE, maxiter = 20)
  mdl$solve(options = list(report = "none"))
  expect_equal(mdl$get_solve_status(), "OK")
  report <- capture.output(mdl$solve())
  expect_known_output(cat_report(convert_report(report,
