...
test_that("by_period FALSE", {
  mdl1 <- mdl$copy()
  mdl1$run_eqn(names = "x2")
  mdl1$run_eqn(names = c("x0", "x1"),
               # check if per = solve_per also works
               by_period = FALSE, forwards = FALSE,  per = solve_per)
...
test_that("period_period TRUE", {
  ...
  mdl1$run_eqn(names = "x2")
  mdl1$run_eqn(names = c("x0", "x1"),
               by_period = TRUE, forwards = FALSE,  period = solve_per)
...
test_that("all equations", {
  mdl1 <- mdl$copy()
  mdl1$run_eqn(by_period = TRUE, forwards = FALSE)
  expected_result <- cbind(x0 = regts(c(3, 3, NA, NA), period  = mdp),
                           x1 = regts(c(2, 2,  2, NA), period = mdp),
...
test_that("period_period TRUE, forwards = TRUE", {
  ...
  mdl1$run_eqn(names = "x2")
  mdl1$run_eqn(names = c("x0", "x1"),
               by_period = TRUE, forwards = TRUE,  period = solve_per)
  expected_result <- cbind(x0 = NA,
                           x1 = regts(c(2, 2,  2, NA), period = mdp),
                           x2 = 1, x4 = NA)
  expect_equal(mdl1$get_data(), expected_result)
...
test_that("error", {
  msg <- "Argument 'by_period' should be a TRUE or FALSE"
  expect_error(mdl$run_eqn(by_period = 2), msg)
  expect_error(mdl$run_eqn(by_period = NA), msg)
  expect_error(mdl$run_eqn(by_period = c(TRUE, TRUE)), msg)
})
