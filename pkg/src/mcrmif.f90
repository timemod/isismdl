...
module mcrmif
  ...
  private :: rdmifh, rdmifc 
  
  contains
  
  subroutine read_mif_file(mdl, modelnmlen, modelnm, reorder_names, ier, fstat)
    use model_type
    use mif_file
    use invpol
    ...
    logical, intent(in)               :: reorder_names
  ...
  &                     ulFunCharCount, mdl%nca, mdl%nendex)
    ...
    if (reorder_names) call sort_names(mdl)
